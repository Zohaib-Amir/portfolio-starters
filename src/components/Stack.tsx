        ...
        <div className="grid lg:grid-cols-2 gap-16">
          <div className="reveal">
            <h2 className="text-3xl md:text-4xl font-bold leading-tight tracking-tight mb-6">
              Tools I use to<br />
              <span className="accent-text">move fast</span> and<br />
              break nothing.
            </h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            {categories.map((cat, i) => (
              <div key={cat.name} className="reveal-right group p-4 rounded-lg border border-border hover:border-accent/20 bg-surface/20 hover:bg-surface-raised transition-all duration-300"
                ...
                <div className="flex flex-wrap gap-1">
                  {cat.items.map((item) => (
                    <span key={item} className="text-[10px] px-1.5 py-0.5 rounded bg-bg text-text-secondary font-mono">{item}</span>
                  ))}
                </div>
              </div>
            ))}
